...
def plot_matrices(frame, backend='myGEMM.cl', WGS = 16, opts='-cl-std=CL1.2'):
    ...
    fig.savefig("plot/plot_matrices.png", dpi=300)
...
def grouped_bar_kernels(frame, backend='myGEMM.cl', WGS = 16, opts='-cl-std=CL1.2'):
    ...
    for kernel in unique_kernels:
        ...
        times = []
        for size in groups:
            times.append(kernel_frame[kernel_frame['matrix_dimensions'] == size]['elapsed_s'].round(2).median())
